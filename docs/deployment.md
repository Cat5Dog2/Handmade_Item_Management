...
gcloud builds submit \
  ...
  --tag asia-northeast1-docker.pkg.dev/your-project-id/handmade-sales-api/api:latest \
  apps/api
```

モノレポ構成により Docker build context を変える場合は、実リポジトリ構成に合わせて調整すること。
...
gcloud run deploy handmade-sales-api \
  ...
  --set-env-vars PORT=8080,API_BASE_PATH=/api,CORS_ORIGIN=https://your-project-id.web.app,GOOGLE_CLOUD_PROJECT=your-project-id,FIREBASE_PROJECT_ID=your-project-id,FIREBASE_STORAGE_BUCKET=your-project-id.appspot.com,SIGNED_URL_EXPIRES_MINUTES=60,IMAGE_MAX_FILE_SIZE_MB=10,IMAGE_MAX_COUNT_PER_PRODUCT=10,IMAGE_DISPLAY_MAX_EDGE_PX=2000,IMAGE_THUMB_MAX_EDGE_PX=400,IMAGE_OUTPUT_FORMAT=webp,IMAGE_WEBP_QUALITY=82,PRODUCT_ID_PREFIX=HM-,PRODUCT_ID_DIGITS=6,PRODUCT_COUNTER_DOCUMENT_PATH=counters/product
```
