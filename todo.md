...
- 優先度: P0
- 依存: `API-BASE-01`
- 現状メモ: Firebase Admin 初期化と認証 middleware はあるが、Firestore / Storage shared client と route 境界整理は未完了。
- 作業:
...
- 現状メモ: `AppError` と error handler はあるが、`{ data, meta }` / `{ code, message, details }` helper 群は未整備。
- 作業:
...
- 優先度: P0
- 依存: `BOOT-01`
- 現状メモ: Vite + React + TypeScript の起動土台と単一画面はあるが、React Router と保護画面ルーティング土台は未実装。
- 作業:
  - [ ] Vite + React + TypeScript を初期化する
  - [ ] React Router を設定する
  - [ ] 基本レイアウトを作成する
  - [ ] 共通ヘッダ / ナビゲーションを配置する
- Done:
  - [ ] `/login` と保護画面のルーティング土台がある
- 推奨コミット:
...
- 作業:
  - [ ] TanStack Query を導入する
  - [ ] React Hook Form + Zod を導入する
  - [ ] API クライアントを実装する
  - [ ] 認証トークン付与処理を実装する
  - [ ] 401 / 403 共通ハンドリングを実装する
- Done:
  - [ ] query / mutation の基盤が利用可能
- 推奨コミット:
...
- 作業:
  - [ ] `/login` 画面を実装する
  - [ ] Firebase Auth ログインを実装する
  - [ ] パスワード再設定導線を実装する
  - [ ] 認証ガードを実装する
  - [ ] ログアウト処理を実装する
  - [ ] ログアウト時の状態破棄を実装する
  - [ ] ログアウト後の初期遷移先を `/dashboard` 前提で統一する
  - [ ] URL に過去の検索条件クエリが残っていても、ログアウト直後の再ログインでは自動復元しない
- Done:
  - [ ] ログイン後に `/dashboard` へ遷移する
  - [ ] 未ログインで保護画面へ入れない
  - [ ] ログアウト直後の再ログインで一覧条件が復元されない
- 推奨コミット:
...
- 作業:
  - [ ] `GET /api/categories` を実装する
  - [ ] `usedProductCount` / `isInUse` を返す
- Done:
  - [ ] カテゴリ一覧が `usedProductCount` / `isInUse` を含めて返る
- 推奨コミット:
...
- 依存: `MASTER-01A`, `API-BASE-02`
- 作業:
  - [ ] `PUT /api/categories/:categoryId` を実装する
  - [ ] 重複名チェックを実装する
- Done:
  - [ ] カテゴリ更新が仕様どおり動く
- 推奨コミット:
...
- 依存: `MASTER-01A`, `API-BASE-02`
- 作業:
  - [ ] `DELETE /api/categories/:categoryId` を実装する
  - [ ] 未使用カテゴリのみ削除可を実装する
- Done:
  - [ ] カテゴリ削除が仕様どおり動く
- 推奨コミット:
...
- 作業:
  ...
  - [ ] `usedProductCount` / `isInUse` を返す
- Done:
  - [ ] タグ一覧が `usedProductCount` / `isInUse` を含めて返る
- 推奨コミット:
...
- 作業:
  - [ ] `package-lock.json` を正本に `npm audit` と `npm audit --omit=dev` を再実行する
  - [ ] `vite` / `vitest` / `vite-node` / `@vitest/mocker` / `esbuild` の dev-only 脆弱性を棚卸しする
  - [ ] 非破壊で更新できる範囲を確認し、難しい場合はメジャー更新の分離方針を整理する
  - [ ] dev server をローカル限定で使う運用条件と、更新見送り条件を docs に反映する
- Done:
  - [ ] dev-only 脆弱性の対象パッケージ、影響範囲、更新方針が文書化されている
  - [ ] 更新した場合は `lint` / `typecheck` / `test` / `build` が通る
  - [ ] 更新を見送る場合も、ローカル限定運用の条件が docs に残っている
- 推奨コミット:
...
現時点のリポジトリでは、**`BOOT-01` / `BOOT-02` / `BOOT-03` / `SHARED-01` / `SHARED-02` / `SHARED-03` / `API-BASE-01` は完了**、**`API-BASE-02` / `API-BASE-03` / `WEB-BASE-01` は着手済み** である。  
そのため、**新規の `[ ]` タスクへ進む前に、最上位の `[~]` タスクである `API-BASE-02` を再開**する。  
...
- 現在の完了: `BOOT-01`, `BOOT-02`, `BOOT-03`, `SHARED-01`, `SHARED-02`, `SHARED-03`, `API-BASE-01`
- 現在の再開候補: `API-BASE-02`, `API-BASE-03`, `WEB-BASE-01`
- 現在の最優先: `API-BASE-02`
