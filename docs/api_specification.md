...
| `isPrimary` | boolean | 任意 | true の場合は追加画像を代表画像として登録する |
...
- `isPrimary=true` の場合、追加画像を `isPrimary=true` とし、他画像は `false` に正規化する
- `isPrimary` 省略時または `false` の場合、追加画像は `isPrimary=false` とする
