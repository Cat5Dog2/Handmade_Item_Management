...
| SCR-04 | 商品詳細画面 | 商品詳細確認 | スマートフォン / PC |
...
- 遷移元: なし
- 遷移先:
  - 認証成功: SCR-02 ダッシュボード画面
  - パスワード再設定導線: 外部または認証基盤の再設定フロー
...
### 6.4.1 目的
商品の詳細情報、画像、タスク、QRコードを確認し、関連タスクの完了状態を直接更新する。
...
| 7 | ステータス | 表示 | バッジ表示 |
| 8 | 販売日時 | 表示 | 販売済時のみ表示 |
| 9 | 代表画像 | 画像 | 未設定時は先頭画像またはプレースホルダー |
| 10 | 画像一覧 | サムネイル一覧 | 複数画像表示 |
| 11 | 関連タスク一覧 | リスト | 未完了を優先表示し、各タスク行から完了状態を直接切り替え可能 |
| 12 | 完了済み表示切替 | スイッチ/タブ | 完了タスクの表示切替 |
| 13 | QRコード | 表示 | 商品識別用 |
| 14 | QR読み取りボタン | ボタン | QRコード読み取り画面へ遷移 |
| 15 | 編集ボタン | ボタン | 商品編集へ遷移 |
| 16 | 削除ボタン | ボタン | 商品削除確認を表示 |
| 17 | タスク管理ボタン | ボタン | タスク管理画面へ遷移 |
...
| 5 | 販売済更新ボタン | ボタン | 更新可能時のみ表示 |
| 6 | 再試行ボタン | ボタン | 失敗時や完了後の再読取用 |
...
### 6.9.6 操作
- QR読取成功時、商品特定処理を実行
- 対象商品の状態が更新可能であれば販売済更新ボタンを表示または活性化する
...
| 4 | 現在ステータス | 展示中または在庫中 |
| 5 | 実行確認文言 | 販売済へ更新する旨 |
| 6 | 確定ボタン | 販売済更新実行。送信中は押下不可 |
| 7 | キャンセルボタン | ダイアログを閉じる。送信中は押下不可 |
...
- 戻る導線
- ログアウト導線
- 必要に応じて保存ボタンや新規追加ボタンを表示

## 7.2 共通ナビゲーション
モバイルでは下部ナビゲーション、PCでは上部または左メニューを想定する。
...
- 入力エラー: 項目近傍
- 処理成功: トーストまたは画面上部通知
- 処理失敗: 画面上部または対象領域にエラー表示
- 認証切れ（401）: 「セッションが切れました。再度ログインしてください。」を表示後、ログイン画面へ遷移
- 利用不可（403）: 「この操作は実行できません。」を表示後、ログイン画面へ遷移
...
| 商品基本情報 / 画像 / タスク / QR表示 | SCR-04 |
| 商品登録 / 編集 / 画像登録 / 代表画像設定 / ステータス変更 | SCR-05 |
| タスク登録 / 編集 / 完了切替 / 削除 | SCR-06 |
| カテゴリ管理 | SCR-07 |
| タグ管理 | SCR-08 |
| QR読み取り / 商品特定 / 販売済更新 | SCR-09, SCR-10 |
| 削除確認 / 販売済戻し確認 | SCR-11 |
