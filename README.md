...
> 補足:
> - 現在のこのリポジトリは、設計書・Firebase設定・Docker雛形を先に置いた初期セットです。
> - ルート `package.json` や各 workspace の `package.json` は、実装開始時に追加する前提です。
> - そのため、`npm install`、`npm run dev`、`docker compose up --build` などの実行系コマンドは、workspace 基盤追加後に利用してください。
...
- ルート `package.json` と workspace 設定があること
- `apps/web`, `apps/api`, `packages/shared` の `package.json` があること
- 現在のこの初期セットには上記ファイルがまだ含まれていない場合があるため、不足時は先に追加すること
...
workspace script を整備したうえで、最低限次を用意してください。
...
- Web から API へは `VITE_API_BASE_URL` 経由で接続し、ローカル既定値は `http://localhost:8080/api` とする

## 6. workspace 基盤作成時の推奨 npm scripts

Codex が迷わないよう、少なくとも以下の script を用意してください。
