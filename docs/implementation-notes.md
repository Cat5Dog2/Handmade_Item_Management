...
- QR印刷では商品詳細画面で生成したQRを再利用し、QR画像そのものは永続保存しない
- 商品詳細のQR印刷は表示中の商品QRを1〜30枚から選択し、A4縦向き、3列×10行、1面あたり横6.5cm×縦2.5cmの30面レイアウトで出力する
- 各印刷面ではQRコードを左側に配置し、商品名の上辺をQRコードの黒いコード部分の上辺に揃え、商品IDは商品名の下に表示する
- 商品一覧のまとめて印刷では、一覧上で選択した異なる商品1〜30件の `productId` を `qrCodeValue` としてQRを生成し、同じ30面レイアウトで出力する
- QR画像そのものを永続保存しない
