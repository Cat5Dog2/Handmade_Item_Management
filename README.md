...
現時点の次期課題:
- ひらがな / カタカナの相互吸収は MVP では未対応です。検索正規化の詳細は `docs/implementation-notes.md` を参照してください。
...
- 顧客IDは `counters/customer` をトランザクション更新して `cus_000001` 形式で採番
...
- 購入者紐付けは `products.soldCustomerId` / `soldCustomerNameSnapshot` で保持
- 顧客別購入履歴は `products.status=sold` と `soldCustomerId` から導出
