...
- スマートフォンとPCで機能差は設けない
- PWAとしてホーム画面にインストールしてネイティブアプリライクに利用できる前提とする
...
- モバイル下部ナビゲーションはアイコン + 短いラベルで1段表示し、主要導線のタップ領域を確保する
- ログアウト導線はダッシュボードページ下部右側に小さく表示する
...
| 状態 | 表示方針 | 主な対象画面 |
|---|---|---|
| 初期読み込み中 | スピナーまたはスケルトンを表示 | 一覧、詳細、ダッシュボード、管理画面 |
| 再取得中 | 直前のリスト状態を維持 | 一覧、顧客一覧 |
...
### 6.1.1 目的
利用者本人の認証を行う。demo 環境ではゲストによる試用を開始できる。
...
| 4 | ゲストログインボタン | ボタン | - | demo 環境のみ表示。匿名認証を実行し、認証中は押下不可 |
| 5 | パスワード再設定リンク | リンク | - | 再設定導線 |
| 6 | エラーメッセージ | メッセージ | - | 認証失敗時に表示 |
...
### 6.1.6 操作
- ログインボタン押下で認証実行
- ゲストログインボタン押下で Firebase Anonymous Authentication を実行する
- Enterキーでもログイン可能とする
...
- 認証中はログインボタンとゲストログインボタンを押下不可とし、二重送信を防止する
...
| No | 項目名 | 種別 | 説明 |
|---|---|---|---|
| 1 | ステータス別件数 | カード | 制作中〜販売済の件数 |
| 2 | 商品合計 | カード | ステータス別件数の合計 |
| 3 | 残タスク | カード | 未完了タスクの合計件数 |
| 4 | 納期が近いタスク一覧 | リスト | 当日含む7日以内の未完了タスク |
| 5 | 最近更新した商品 | リスト | 更新日時降順、最大5件 |
| 6 | 特注一覧 | リスト | 特注フラグが有効な商品を更新日時降順で表示 |
| 7 | 再試行ボタン | ボタン | 通信失敗時 |
...
- 最近更新した商品は最大5件
- 特注一覧は `isCustomOrder=true` の商品を表示する
...
### 6.3.3 レイアウト
- 上部: 検索欄、フィルタ群、販売済表示、並び替え
- 中央: 商品カードまたは商品行一覧
- 下部: ページングUI（現在ページ表示、前へ/次へボタン）
- 右下または下部固定: 新規登録ボタン
- 検索条件はキーワードを横幅いっぱいにし、カテゴリ・タグ・ステータス・並び順はコンパクトに並べる。PCでは検索条件を横並び配置
...
| 6 | 販売済表示 | セレクト | 「含める」「含めない」を選択。ステータス絞り込みで「販売済」を選択時は「含める」固定 |
| 7 | 特注 | セレクト | 「すべて」「特注のみ」「特注を含めない」を選択 |
| 8 | 在庫限り | セレクト | 「すべて」「在庫限りのみ」「在庫限りを含めない」を選択 |
| 7 | 商品一覧 | リスト | サムネイル、商品名、商品ID、ステータス、特注表示、在庫限り表示、カテゴリ、更新日時 |
...
| 13 | 再試行ボタン | ボタン | 通信失敗時 |
| 14 | まとめて印刷ボタン | ボタン | 選択モードに入り、異なる商品1〜30件のQRコードをA4縦向き、3列×10行、1面あたり横6.5cm×縦2.5cmで印刷 |

### 6.3.5 初期表示
- 並び順は更新日時降順
- 販売済表示の初期値は「含める」とし、販売済商品を区別表示する
- 検索条件未指定
- 現在ページは1
- 取得件数は1ページあたり既定50件
- ローディング後に一覧表示
...
### 6.3.7 操作
- 商品行タップで商品詳細へ遷移
- まとめて印刷ボタン押下で商品選択モードへ切り替え、1〜30件選択後に選択商品のQRコードを印刷する
- 新規登録ボタン押下で商品登録画面へ遷移
...
- ステータス絞り込みで「販売済」を選択した場合、販売済表示は「含める」固定とし、内部的に販売済商品を表示対象に含める
- 0件時は条件見直し導線を表示する
...
- ひらがな/カタカナは別文字として扱う
- 空文字のみは未指定として扱う
- ステータス絞り込みで「販売済」を選択中は、販売済表示をユーザー操作で「含めない」にできない
...
| 7 | ステータス | 表示 | バッジ表示 |
| 8 | 特注 | 表示 | 特注フラグの有無 |
| 9 | 在庫限り | 表示 | 在庫限りフラグの有無 |
| 10 | 販売日時 | 表示 | 販売済時のみ表示 |
| 11 | 購入者情報 | 表示 | 販売済かつ顧客紐付け済みの場合に購入者名を表示 |
| 12 | 購入者詳細導線 | ボタン/リンク | 顧客紐付け済みの場合に顧客詳細画面へ遷移 |
| 13 | 代表画像 | 画像 | 未設定時は先頭画像またはプレースホルダー |
| 14 | 画像一覧 | サムネイル一覧 | 複数画像表示 |
| 15 | 関連タスク一覧 | リスト | 未完了を優先表示し、各タスク行から完了状態を直接切り替え可能 |
| 16 | 完了済み表示切替 | スイッチ/タブ | 完了タスクの表示切替 |
| 17 | QRコード | 表示 | 商品識別用 |
| 18 | QR読み取りボタン | ボタン | QRコード読み取り画面へ遷移 |
| 19 | QR印刷枚数 | セレクト | 1〜30枚から選択。初期値は1枚 |
| 20 | QR印刷ボタン | ボタン | 表示中のQRコードを選択枚数分、A4縦向き、3列×10行、1面あたり横6.5cm×縦2.5cmで印刷 |
| 21 | 編集ボタン | ボタン | 商品編集へ遷移 |
| 22 | 削除ボタン | ボタン | 商品削除確認を表示 |
| 23 | タスク管理ボタン | ボタン | タスク管理画面へ遷移 |
...
- QR印刷枚数は1〜30枚から選択できる
- QR印刷ボタン押下で表示中のQRコードを選択枚数分、A4縦向き、3列×10行、1面あたり横6.5cm×縦2.5cmで印刷する
- 編集ボタン押下で商品編集画面へ遷移
...
| 9 | ステータス | セレクト | ○ | 1件選択 |
| 10 | 特注フラグ | チェックボックス | - | 特注商品として扱うか |
| 11 | 在庫限りフラグ | チェックボックス | - | 在庫限りの商品として扱うか |
| 12 | 保存ボタン | ボタン | - | 登録/更新実行 |
| 13 | キャンセルボタン | ボタン | - | 前画面へ戻る |
| 14 | 入力エラー表示 | メッセージ | - | 項目ごと/画面上部表示 |
...
#### 編集時
- 登録済み情報を表示
- 画像、代表画像、ステータス、タグ、特注フラグ、在庫限りフラグも既存値を反映
...
### 6.6.6 操作
- 追加ボタン押下で新規タスクフォームを表示し、フォーム位置へスクロールして入力欄へ案内する
...
| 3 | 読み取り状態表示 | バッジ/メッセージ | 起動中、読み取り中、照合中などの状態表示 |
| 4 | 読取結果メッセージ | メッセージ | 成功/失敗/対象外表示 |
| 5 | 商品確認情報 | 表示 | 商品名、商品ID、ステータスなど |
| 6 | 購入者選択 | セレクト/オートコンプリート | 更新可能時のみ表示。未選択も可 |
| 7 | 販売済更新ボタン | ボタン | 更新可能時のみ表示 |
| 8 | 再試行ボタン | ボタン | 失敗時や完了後の再読取用 |
...
| 3 | 年代 | 選択 | - | 任意 |
| 4 | 系統メモ | 入力 | - | 最大100文字 |
| 5 | SNSアカウント | 繰り返し入力 | - | platformは選択式。urlはplatform / accountNameから自動入力。noteは任意 |
...
|---|---|
| 認証 | SCR-01 |
| ステータス別件数 / 残タスク / 最近更新商品 / 特注一覧 | SCR-02 |
| 商品検索 / 絞り込み / 並び替え | SCR-03 |
| 商品基本情報 / 画像 / タスク / 購入者情報 / QR表示 / QR読み取り導線 | SCR-04 |
| 商品登録 / 編集 / 画像登録 / 代表画像設定 / ステータス変更 / 特注フラグ / 在庫限りフラグ / 購入者設定 | SCR-05 |
