...
| データベース | Firestore | 商品、タスク、カテゴリ、タグ等の保存 |
...
| 通信 | 方式 | 用途 |
|---|---|---|
| フロント ↔ API | HTTPS / JSON | 商品、タスク、カテゴリ、タグ、ダッシュボード、QR販売更新 |
...
| SCR-04 | 商品詳細画面 | 商品情報、画像、タスク、QR表示 |
...
- 商品一覧の検索結果が0件の場合は「該当する商品はありません」を表示し、条件クリア導線を併設する
...
- 商品基本情報更新は `PUT /api/products/:productId` で行い、代表画像の変更は `primaryImageId` 指定として受け付ける
...
```json
{
  ...
  "images": [
    {
      "imageId": "img_001",
      "displayUrl": "https://.../display/img_001.webp",
      "thumbnailUrl": "https://.../thumb/img_001.webp",
      "sortOrder": 1,
      "isPrimary": true
    }
  ],
...
- FirestoreのドキュメントIDとは分離してもよいが、MVPでは一致させてもよい
...
- 論理削除済み商品に紐づく通常参照APIは、業務エラーとして参照不可を返却する
...
| `PRODUCT_DELETED` | 400 | 論理削除済み |
| `PRODUCT_RELATED_RESOURCE_UNAVAILABLE` | 400 | 論理削除済み商品の関連情報参照不可 |
...
- 画面レイアウト詳細
- APIリクエスト/レスポンス定義
- Firestoreインデックス定義の最終版
- 画像変換方式、命名規則、保存パス
- QRコード生成ライブラリ、読み取りライブラリ
- フロント状態管理方式
- エラーメッセージ文言一覧
- ログ出力項目と保持期間
