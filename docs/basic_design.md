...
- アプリ利用者本人のみ。demo 環境では匿名ゲストによる試用を例外として許可する
- 複数ユーザー利用は対象外
...
| 区分 | 採用技術 | 役割 |
|---|---|---|
| フロントエンド | React + TypeScript + Vite | 画面表示、入力、カメラ利用、QR読取、状態管理、PWA対応（マニフェスト・アイコン提供） |
...
- demo 環境では匿名認証プロバイダーのトークンを設定により例外許可する
- 未認証時は管理画面にアクセス不可とする
- 所有者と demo 匿名ゲストの権限差は設けず、ロール管理は行わない
...
| 状態 | 対象画面 | 表示方針 | 利用者導線 |
|---|---|---|---|
| 初期読み込み中 | ダッシュボード、商品一覧、商品詳細、QR読取結果、各管理画面 | スピナーまたはスケルトンを表示し、二重送信を防ぐ | 読み込み完了まで待機 |
| 条件変更による再取得中 | 商品一覧、顧客一覧 | スピナー等は表示せず、直前のリスト状態を保持したまま再取得を行う | 読み込み完了まで直前の状態で待機 |
...
利用者本人の認証を行う。demo 環境では匿名ゲストによる試用も受け付ける。
...
- demo 環境のゲストログインでは Firebase Anonymous Authentication を使用する
...
- ゲストログインボタンは Web と API の demo 専用設定が有効な場合のみ表示・許可する
...
補足:
- 要件定義書内の「未完了タスク一覧」という表現揺れは、本設計では「残タスク」と「納期が近いタスク一覧」に分けて整理する
...
- まとめて印刷では商品一覧上で異なる商品を1〜30件選択し、選択した商品のQRコードをA4縦向き、3列×10行、1面あたり横6.5cm×縦2.5cmで印刷する
...
- QR印刷枚数は1〜30枚から選択できる
- QR印刷ボタンから表示中のQRコードを選択枚数分、A4縦向き、3列×10行、1面あたり横6.5cm×縦2.5cmで印刷できる
...
- 販売済から戻す場合は確認を必須とする
- 販売済から委託販売 / マルシェ / 在庫中 / その他ステータスへ戻した場合は `soldAt` を `null` に更新する
...
- 同一商品には同一QRコードを利用する
- QR印刷は商品詳細画面で表示中の商品QRを1〜30枚から選択し、A4縦向き、3列×10行、1面あたり横6.5cm×縦2.5cmで出力する
- 商品一覧のまとめて印刷では異なる商品1〜30件のQRを同じ30面レイアウトで出力する
...
- 論理削除済み商品は除外
- 論理削除済み商品に紐づくタスクも除外
- 残タスクは、論理削除されていない商品に紐づく未完了タスクを対象に集計する
...
- 通信はHTTPSとする
- 通常環境は認証済み本人のみ利用可能とし、demo 環境だけ匿名ゲストを設定により例外許可する
...
- demo 環境に限り、`ENABLE_GUEST_LOGIN=true` かつ ID トークンの `firebase.sign_in_provider=anonymous` の場合を例外として許可する
- Web 側の `VITE_ENABLE_GUEST_LOGIN` は表示制御にのみ使い、API 側の `ENABLE_GUEST_LOGIN` を認可の正本とする
...
- 商品は一点物で数量概念を持たない
- 通常の利用者は本人のみとし、demo 環境では匿名ゲストによる試用を許可する
- 商品履歴管理はMVP対象外である
