...
本アプリは通常環境では**利用者本人のみ**を想定し、demo 環境だけ匿名ゲストを許可する。
...
- demo Firebase プロジェクトだけ Anonymous 認証を有効化する。stg / prod では有効化しない
...
- `VITE_ENABLE_GUEST_LOGIN`（demo デプロイスクリプトだけ `true`）
...
- `API_BASE_PATH`
- `APP_OWNER_EMAIL`
- `ENABLE_GUEST_LOGIN`（demo デプロイスクリプトだけ `true`）
- `CORS_ORIGIN`
...
demo 用スクリプトは `_ENABLE_GUEST_LOGIN=true` を Cloud Build へ渡し、Web の `VITE_ENABLE_GUEST_LOGIN` と Cloud Run の `ENABLE_GUEST_LOGIN` を同時に有効化する。stg / prod 用スクリプトは両方を `false` に固定する。
...
- demo 環境では匿名ゲストが利用でき、stg / prod では同じ匿名トークンが 403 になること
...
- 顧客登録 / 編集 / アーカイブができる
- 顧客IDが `cus_000001` 形式で採番される
- アーカイブ済み顧客が通常一覧と購入者候補から除外される
- 商品編集で販売済商品の購入者を任意に紐付けできる
- 顧客別購入履歴が `products.status=sold` と `soldCustomerId` から表示される
