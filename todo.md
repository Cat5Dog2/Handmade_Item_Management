...
- 作業:
  - [ ] `GET /api/customers/:customerId/purchases` を実装する
  - [ ] `products.status=sold` かつ `soldCustomerId=:customerId` から導出する
  - [ ] 別 `sales` コレクションを作らない
- Done:
  - [ ] 顧客別購入履歴を販売日時降順で取得できる
- 推奨コミット:
...
- 作業:
  - [ ] `/customers/:customerId` を実装する
  - [ ] 顧客基本情報 / 購入サマリ / 購入履歴を表示する
  - [ ] 編集 / アーカイブ導線を実装する
- Done:
  - [ ] 顧客詳細と購入履歴を確認できる
- 推奨コミット:
...
- 作業:
  - [ ] `/customers/new` を実装する
  - [ ] `/customers/:customerId/edit` を実装する
  - [ ] 顧客入力フォームと validation を実装する
  - [ ] 保存成功後の詳細遷移を実装する
- Done:
  - [ ] 顧客の登録 / 編集が UI から完了できる
- 推奨コミット:
...
- 作業:
  ...
  - [ ] `sold` の重複更新防止を実装する
  - [ ] 任意 `customerId` を受け取り、未指定でも販売済更新できる
  - [ ] 顧客指定時は存在する未アーカイブ顧客のみ許可する
  - [ ] `soldCustomerId` / `soldCustomerNameSnapshot` を更新し、レスポンスへ返す
- Done:
  - [ ] QR 販売済更新の業務ルールが通る
  - [ ] QR 販売済更新時の任意顧客紐付けが仕様どおり動く
- 推奨コミット:
