...
### 1.3 前提
- 利用者はアプリ利用者本人のみ。ただし demo 環境では匿名ゲストによる試用を許可する
...
- API はトークン検証後、通常は `APP_OWNER_EMAIL` と一致するメールアドレスのみ許可する。demo 環境では `ENABLE_GUEST_LOGIN=true` かつ匿名認証プロバイダーのトークンを例外許可する
...
- API はトークン検証後、`APP_OWNER_EMAIL` と一致するメールアドレス、または `ENABLE_GUEST_LOGIN=true` の環境で `firebase.sign_in_provider=anonymous` を持つユーザーのみ許可し、それ以外は `403` を返す
...
- MVPで `meta` を返すのは `GET /api/products` と `GET /api/customers` とする
...
| `CUSTOMER_NOT_FOUND` | 404 / 400 | 顧客が存在しない。詳細取得では `404`、商品更新やQR販売済更新で指定不正時は `400` |
...
- demo 環境に限り Firebase Anonymous Authentication を併用する
- フロントエンドは Firebase SDK で認証し、取得した ID トークンを API に付与する
- API はトークンを検証し、認証済み要求のみ処理する
- API はトークン検証後、所有者のメールアドレス一致、または demo 環境で許可された匿名認証であることを確認し、それ以外は `AUTH_FORBIDDEN` を返す
...
### 業務ルール
- フロントは `signInWithEmailAndPassword` または demo 環境の `signInAnonymously` 成功後、ID トークン取得後に本 API を 1 回呼ぶ
- 本 API は所有者の allowlist 一致、または demo 環境で許可された匿名認証の要求のみ受け付ける
...
| Customers | GET | `/api/customers` | 200 OK | 顧客一覧取得 |
| Customers | POST | `/api/customers` | 201 Created | 顧客登録 |
| Customers | GET | `/api/customers/:customerId` | 200 OK | 顧客詳細取得 |
| Customers | PUT | `/api/customers/:customerId` | 200 OK | 顧客更新 |
| Customers | DELETE | `/api/customers/:customerId` | 200 OK | 顧客アーカイブ |
| Customer Purchases | GET | `/api/customers/:customerId/purchases` | 200 OK | 顧客別購入商品一覧取得 |
...
- `status=sold` かつ `soldCustomerId` 指定時は、存在する未アーカイブ顧客のみ設定可とする
- `status=sold` かつ `soldCustomerId=null` の場合は顧客未設定の販売済として扱う
- `soldCustomerId` 指定時は `soldCustomerNameSnapshot` を保存時に更新する
- `sold` から他ステータスへ戻す場合は `soldAt=null`、`soldCustomerId=null`、`soldCustomerNameSnapshot=null` とする
...
| パラメータ | 型 | 必須 | 説明 |
|---|---|---|---|
| `page` | number |  | 1以上のページ番号。既定値 `1` |
| `pageSize` | number |  | 1〜100。既定値 `50` |
| `keyword` | string |  | 顧客名、SNSアカウント、メモを対象に検索 |
| `sortBy` | string |  | `updatedAt` / `lastPurchaseAt` / `name` |
| `sortOrder` | string |  | `asc` / `desc` |

### 業務ルール
- `lastPurchaseAt`、`lastPurchaseProductId`、`lastPurchaseProductName`、`purchaseCount` は `products.status=sold` かつ `soldCustomerId` 一致の販売済商品から導出する
- 顧客本体に購入履歴サマリを永続保持しない
- 内部項目名は `ageGroup` / `customerStyle` を使用し、`ageRange` / `styleTags` は採用しない
...
### 業務ルール
- 顧客IDは `counters/customer` をトランザクション更新して `cus_000001` 形式で採番する
- 採番済みの顧客IDは再利用しない
- `name` は必須、最大100文字
- 電話番号、メールアドレスはMVP対象外
- `snsAccounts` は任意配列とする
...
## 12.3 GET /api/customers/:customerId

### 概要
顧客詳細を取得する。

### 業務ルール
- 顧客基本情報に加え、`summary.lastPurchaseAt`、`summary.lastPurchaseProductId`、`summary.lastPurchaseProductName`、`summary.purchaseCount` を返す
- `summary` 配下の項目は `products.status=sold` かつ `soldCustomerId=:customerId` の販売済商品から導出する
- 顧客本体に購入履歴サマリを永続保持しない
- 内部項目名は `ageGroup` / `customerStyle` を使用し、`ageRange` / `styleTags` は採用しない
...
## 12.4 PUT /api/customers/:customerId

### 概要
顧客情報を更新する。

### リクエストボディ
`POST /api/customers` と同形式。

### 業務ルール
- 更新成功時は `updatedAt` を更新する
- 成功時は `operationLogs` に `eventType=CUSTOMER_UPDATED`, `targetId=<customerId>`, `summary=顧客情報を更新しました`, `detail.result=success` を記録する
...
## 12.5 DELETE /api/customers/:customerId

### 概要
顧客をアーカイブする。

### 業務ルール
- 物理削除は行わず `isArchived=true` に更新する
- 販売済商品の履歴参照は保持する
- 成功時は `operationLogs` に `eventType=CUSTOMER_ARCHIVED`, `targetId=<customerId>`, `summary=顧客をアーカイブしました`, `detail.result=success` を記録する
...
## 12.6 GET /api/customers/:customerId/purchases

### 概要
顧客別購入商品一覧を取得する。

### 業務ルール
- `products.status=sold` かつ `soldCustomerId=:customerId` を条件に導出する
- 販売日時降順で返す
...
### 業務ルール
- `status=consignmentSale`、`marche` または `inStock` の場合のみ `canSell=true`
- `sold` は重複更新せず `ALREADY_SOLD`
- `inProduction` / `completed` は更新不可
- 論理削除済み商品は無効QR扱い
...
| `customerId` | string \| null | 任意 | 購入者として紐付ける顧客ID。未選択は `null` または省略可 |
...
- `customerId` は任意で、未選択のままでも販売済更新できる
- `customerId` 指定時は、存在する未アーカイブ顧客のみ更新に利用する
- 対象ステータスは `consignmentSale` / `marche` / `inStock` のみ
- 更新直前に最新状態を再取得して可否判定を再実施する
- `soldAt` 未設定時のみ現在時刻を設定する
- `customerId` 指定時は `soldCustomerId` と `soldCustomerNameSnapshot` を更新する
- `customerId` 未指定時は顧客未設定の販売済として更新する
...
| `soldCustomerId` | string \| null | 購入者顧客ID |
| `soldCustomerNameSnapshot` | string \| null | 購入者名スナップショット |
...
| `name` | string | 顧客名 |
| `normalizedName` | string | 検索用正規化名 |
| `gender` | string \| null | 性別 |
| `ageGroup` | string \| null | 年代 |
| `customerStyle` | string \| null | 系統メモ |
| `snsAccounts` | object[] | SNSアカウント一覧 |
| `memo` | string \| null | 顧客メモ |
