...
- 顧客別購入履歴は `products.status=sold` と `soldCustomerId` から導出し、別 `sales` コレクションは作成しない
...
erDiagram
    ...
    CUSTOMERS ||--o{ PRODUCTS : purchases
...
| `customers` | 顧客マスタ | `customerId` | アーカイブ |
...
| `customers` | `customers/{customerId}` | 内部IDを採用 |
| `counters` | `counters/{counterKey}` | `counters/product`、`counters/customer` を使用 |
...
| `soldCustomerId` | string\|null |  | 購入者顧客ID | `customers.customerId` を参照。未設定可 |
| `soldCustomerNameSnapshot` | string\|null |  | 購入者名スナップショット | 販売時点の顧客名を保持 |
...
- `status=sold` かつ `soldCustomerId` 指定時は存在する未アーカイブ顧客のみ設定できる
- `soldCustomerId` 指定時は `soldCustomerNameSnapshot` を顧客名で更新する
- 既に `soldAt` が設定済みの販売済商品を再度販売済にしても上書きしない
- `sold` から他ステータスへ戻す場合は `soldAt=null`、`soldCustomerId=null`、`soldCustomerNameSnapshot=null` にする
...
- 顧客別購入履歴は `products.status=sold` かつ `soldCustomerId=<customerId>` を条件に導出する
- 顧客の最終購入日、最終購入商品、購入回数は一覧取得時に導出または集計して返す
...
| 項目 | 型 | 必須 | 説明 | 制約・補足 |
|---|---|---:|---|---|
| `customerId` | string | ○ | 顧客ID | `counters/customer` で採番する `cus_000001` 形式の内部ID |
| `name` | string | ○ | 顧客名 | 最大100文字、前後空白除去 |
| `normalizedName` | string | ○ | 検索用顧客名 | name 正規化値 |
| `gender` | string\|null |  | 性別 | 任意 |
| `ageGroup` | string\|null |  | 年代 | 任意 |
| `customerStyle` | string\|null |  | 系統メモ | 任意、最大100文字 |
| `snsAccounts` | array<object> |  | SNSアカウント一覧 | `platform`, `accountName`, `url`, `note` を保持 |
| `memo` | string\|null |  | 顧客メモ | 最大1000文字 |
| `isArchived` | boolean | ○ | アーカイブフラグ | 通常一覧は `false` のみ対象 |
| `archivedAt` | timestamp\|null |  | アーカイブ日時 | アーカイブ時に設定 |
| `createdAt` | timestamp | ○ | 作成日時 | UTC Timestamp |
| `updatedAt` | timestamp | ○ | 更新日時 | 保存成功時に更新 |

### 11.2 業務ルール
- 顧客新規登録時にトランザクションで `counters/customer.currentValue` をインクリメントする
- 採番結果を `cus_000001` 形式へ整形して `customerId` とする
- アーカイブ済み顧客であっても採番済みIDは再利用しない
- 顧客名は必須とし、前後空白除去後に空文字不可とする
- 顧客アーカイブ時は `isArchived=true`、`archivedAt=現在時刻`、`updatedAt=現在時刻` とする
- アーカイブ済み顧客は通常一覧に表示しない
- アーカイブ済み顧客は商品更新やQR販売済更新の紐付け先に指定できない
...
商品IDと顧客IDの採番を安全に行うための連番管理データを保持する。
...
- 商品新規登録時に `counters/product.currentValue` をトランザクションでインクリメントする
- 採番結果を `HM-000001` 形式へ整形して `productId` とする
- 論理削除済み商品であっても採番済みIDは再利用しない
- 顧客新規登録時に `counters/customer.currentValue` をトランザクションでインクリメントする
- 採番結果を `cus_000001` 形式へ整形して `customerId` とする
- アーカイブ済み顧客であっても採番済みIDは再利用しない
...
| `customOrderProducts` | 保存しない | 特注フラグが有効な商品一覧 |
...
| インデックス | 用途 |
|---|---|
| `isArchived + updatedAt` | 顧客一覧 |
| `isArchived + normalizedName + updatedAt` | 顧客検索 |
| `isDeleted + status + soldCustomerId + soldAt` | 顧客別購入履歴 |
...
- `isArchived=false` を前提に通常一覧を構成する
- 顧客名は `normalizedName` を活用して前段絞り込みする
- SNSアカウントやメモは API 側の後段フィルタで補完する
...
- 特注一覧は `isCustomOrder=true` の商品を更新日時降順で表示する
...
| Tag | `tagId` | 商品に複数設定可能 |
| Customer | `customerId` | 顧客本体。アーカイブ運用 |
