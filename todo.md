...
- 依存: `TASK-01A`, `API-BASE-02`
- 作業:
  - [x] `DELETE /api/tasks/:taskId` を実装する
  - [x] タスク存在チェックを実装する
- Done:
  - [x] タスク削除が仕様どおり動く
- 推奨コミット:
...
Status: Done
Changed files:
- apps/api/src/tasks/delete-task.ts
- apps/api/src/tasks/delete-task.test.ts
- apps/api/src/routes/tasks.ts
- apps/api/src/routes/tasks.test.ts
- todo.md
Checks:
- npm --workspace apps/api run test -- delete-task.test.ts routes/tasks.test.ts
- npm run typecheck
- npm run lint:api
- npm run lint
- npm run test
...
- npm run build:api
Notes:
- `DELETE /api/tasks/:taskId` を認証必須ルートとして接続した。
- 削除前にタスク存在確認と関連商品の論理削除確認を行い、成功時は `tasks` ドキュメントを物理削除する。
- タスク不存在は `TASK_NOT_FOUND`、論理削除済み商品配下は `PRODUCT_RELATED_RESOURCE_UNAVAILABLE` として扱う。
Next:
- TASK-02A
```
...
- 作業:
  - [x] `/products/:productId/tasks` を実装する
  - [x] 一覧表示を実装する
  - [x] 商品詳細からの遷移導線を接続する
- Done:
  - [x] 商品詳細 → タスク管理画面の参照導線がつながる
- 推奨コミット:
...
- apps/web/src/products/product-task-management-page.tsx
- apps/web/src/products/product-task-management-page.test.tsx
- apps/web/src/styles.css
- todo.md
Checks:
- npm --workspace apps/web run test -- product-task-management-page.test.tsx App.test.tsx
- npm run typecheck
- npm run lint:web
- npm run lint
- npm run test
- npm run build:web
Notes:
- `/products/:productId/tasks` にタスク管理の参照画面を接続し、対象商品名・商品ID・商品詳細への戻り導線を表示する。
- 初期表示は `GET /api/products/:productId/tasks?showCompleted=false` で未完了タスクのみ取得し、一覧、0件、通信失敗、商品不存在を表示する。
- 追加 / 編集 / 削除 UI と完了切替 / 完了済み表示切替は後続の `TASK-02B` / `TASK-02C` に残す。
Next:
- TASK-02B
```
...
- todo.md
Checks:
- npm --workspace apps/web run test -- product-task-management-page.test.tsx
- npm run typecheck
- npm run lint
- npm run test
- npm run build:web
- git diff --check
Notes:
- `/products/:productId/tasks` にタスク追加フォーム、既存タスク編集、削除確認ダイアログを追加した。
- 作成 / 更新 / 削除後は対象商品のタスク一覧、商品詳細のタスク概要、ダッシュボード集計を再取得する。
- 完了 / 未完了切替と完了済み表示切替は `TASK-02C` に残す。
Next:
- TASK-02C
```
...
- 作業:
  - [x] 完了済み表示切替を実装する
  - [x] 完了 / 未完了切替 UI を実装する
  - [x] 商品詳細上の簡易表示と責務が重複しないことを確認する
- Done:
  - [x] 商品単位のタスク管理が画面として完結する
- 推奨コミット:
...
- git diff --check
Notes:
- タスク管理画面に完了済み表示切替を追加し、`GET /api/products/:productId/tasks` の `showCompleted` 条件で一覧を再取得する。
- 各タスクに完了 / 未完了チェックを追加し、`PATCH /api/tasks/:taskId/completion` 後に一覧とタスク概要件数を同期する。
- 商品詳細は簡易切替、タスク管理画面は登録・編集・削除を含む管理画面として責務を分けた。
Next:
- IMAGE-01
```
...
- 依存: `PRODUCT-05A`, `API-BASE-02`
- 作業:
  - [x] `multer` memoryStorage を設定する
  - [x] `sharp` で画像変換を実装する
  - [x] Exif 向き補正を実装する
  - [x] 表示用 2000px WebP を実装する
  - [x] サムネイル 400px WebP を実装する
  - [x] MIME type / 10MB / 10枚上限チェックを実装する
- Done:
  - [x] 画像バッファから display / thumb を生成できる
- 推奨コミット:
...
- apps/api/src/images/product-image-processing.ts
- apps/api/src/images/product-image-processing.test.ts
- package-lock.json
- todo.md
Checks:
- npm --workspace apps/api run test -- product-image-processing.test.ts
- npm run lint
- npm run test
- npm run typecheck
- npm run build:api
...
- git diff --check
Notes:
- `multer` memoryStorage の単一ファイル受け口を追加し、`file` フィールド、10MB制限、JPEG / PNG / WebP MIME判定を共通化した。
- `sharp` で Exif 向き補正後、表示用2000px・サムネイル400pxの WebP バッファを生成する処理を追加した。
- 10枚上限判定と Storage 保存パス生成を追加した。実際のStorage保存、Firestore画像メタデータ更新、画像APIルート接続は `IMAGE-02A` 以降に残す。
Next:
- IMAGE-02A
```
...
- 作業:
  - [x] `POST /api/products/:productId/images` を実装する
  - [x] Storage 保存を実装する
  - [x] 10枚上限チェックを接続する
- Done:
  - [x] API から画像追加ができる
...
- apps/api/src/products/create-product-image.ts
- apps/api/src/products/create-product-image.test.ts
- apps/api/src/routes/products.ts
- apps/api/src/routes/products.test.ts
- todo.md
Checks:
- npm --workspace apps/api run test -- src/products/create-product-image.test.ts src/routes/products.test.ts
- npm run lint
- npm run typecheck
- npm run test
- npm run build:web
- npm run build
Notes:
- `POST /api/products/:productId/images` を追加し、`multer` の `file` 受け口から Storage へ display / thumbnail の WebP を保存するようにした。
- Firestore の `products.images` に `imageId`、`displayPath`、`thumbnailPath`、`sortOrder`、`isPrimary` を追加し、既存画像は保持した。
- 10枚上限は追加前に判定し、競合で後から上限超過した場合は保存済みファイルを削除するようにした。
- 代表画像は画像追加APIでは変更しない。
Next:
- IMAGE-02B
```
...
- 作業:
  - [x] `PUT /api/products/:productId/images/:imageId` を実装する
  - [x] `imageId` / `sortOrder` 維持を実装する
- Done:
  - [x] API から画像差し替えができる
- 推奨コミット:
...
Status: Done
Changed files:
- apps/api/src/products/replace-product-image.ts
- apps/api/src/products/replace-product-image.test.ts
- apps/api/src/routes/products.ts
- apps/api/src/routes/products.test.ts
- todo.md
Checks:
- npm --workspace apps/api run test -- src/products/replace-product-image.test.ts src/routes/products.test.ts
- npm run lint
- npm run typecheck
- npm run test
- npm run build:web
...
- git diff --check
Notes:
- `PUT /api/products/:productId/images/:imageId` を追加し、`imageId` と `sortOrder` を維持したまま表示用・サムネイル用 WebP を上書きするようにした。
- 論理削除済み商品、存在しない商品、存在しない画像、非対応 MIME を `PRODUCT_NOT_FOUND` / `PRODUCT_RELATED_RESOURCE_UNAVAILABLE` / `IMAGE_NOT_FOUND` / `UNSUPPORTED_IMAGE_TYPE` で返すようにした。
- 画像メタ情報は更新せず、`isPrimary` も維持した。
Next:
- IMAGE-02C
```
...
- 依存: `IMAGE-02A`, `API-BASE-02`
- 作業:
  - [x] `DELETE /api/products/:productId/images/:imageId` を実装する
  - [x] Storage 実体削除を実装する
  - [x] `sortOrder` 詰め直しを実装する
  - [x] 代表画像制御を実装する
- Done:
  - [x] API から画像削除後の整合が保たれる
- 推奨コミット:
...
Status: Done
Changed files:
- apps/api/src/products/delete-product-image.ts
- apps/api/src/products/delete-product-image.test.ts
- apps/api/src/routes/products.ts
- apps/api/src/routes/products.test.ts
- todo.md
Checks:
- npm --workspace apps/api run test -- src/products/delete-product-image.test.ts src/routes/products.test.ts
- npm run lint
- npm run typecheck
- npm run test
- npm run build:web
...
- git diff --check
Notes:
- `DELETE /api/products/:productId/images/:imageId` を追加し、画像実体削除と商品画像配列の更新を行うようにした。
- 削除時は残画像を `sortOrder` 昇順で 1 から詰め直し、削除対象が代表画像だった場合は先頭画像を代表扱いにした。
- 画像未存在、商品未存在、論理削除済み商品を `IMAGE_NOT_FOUND` / `PRODUCT_NOT_FOUND` / `PRODUCT_RELATED_RESOURCE_UNAVAILABLE` で返すようにした。
Next:
- IMAGE-03A
```
...
- todo.md
Checks:
- npm --workspace apps/web run test -- src/products/product-edit-page.test.tsx
- git diff --check
- npm run lint
- npm run typecheck
- npm run build:web
- npm run test
Notes:
- 商品編集画面に画像追加 / 差し替えセクションを追加した。
- 画像操作は `multipart/form-data` の `file` を使い、成功後は商品詳細を再取得して一覧を更新する。
- 画像上限、非対応形式、サイズ超過、論理削除済み商品のエラー文言を画面側で日本語化した。
Next:
- IMAGE-03B
```
...
- git diff --check
Notes:
- 商品編集画面に代表画像の選択セレクトと画像削除ボタンを追加した。
- 削除後は商品詳細を再取得して、代表画像の選択と画像一覧を最新状態へ合わせるようにした。
- `primaryImageId` のサーバーエラーを代表画像欄へ表示するようにした。
Next:
- QR-02C
```
...
- 作業:
  - [x] `/qr` を実装する
  - [x] `html5-qrcode` で読み取りを実装する
  - [x] 多重読み取り防止を実装する
  - [x] lookup 結果表示を実装する
- Done:
  - [x] QR 読み取りから lookup 結果表示までつながる
- 推奨コミット:
...
- apps/web/src/App.test.tsx
- apps/web/src/messages/display-messages.ts
- apps/web/src/qr/qr-page.tsx
- apps/web/src/qr/qr-page.test.tsx
- apps/web/src/qr/qr-scanner-adapter.ts
- apps/web/src/routes/protected-routes.ts
- apps/web/src/styles.css
- apps/web/package.json
- package-lock.json
- todo.md
...
Checks:
- npm --workspace apps/web run test -- src/qr/qr-page.test.tsx src/App.test.tsx
- npm run lint
- npm run typecheck
- npm run test
- npm run build:web
...
Notes:
- `/qr` で `html5-qrcode` を使ったカメラ読み取りと lookup 結果表示を実装した。
- 読み取り中の多重実行を抑止し、lookup 中は scanner を一時停止するようにした。
- lookup 成功時は商品情報と更新可否を表示し、カメラ失敗や lookup エラーは再試行導線付きで表示する。
- `App` のルーティングと保護ナビゲーションに `/qr` を接続した。
...
- 作業:
  - [x] 販売済更新確認ダイアログを実装する
  - [x] canSell=true の場合のみ更新実行できるようにする
  - [x] 購入者選択は任意とし、未選択でも販売済更新できる
  - [x] 更新後の詳細遷移または再読取導線を実装する
- Done:
  - [x] 商品詳細 → QR読取 → 販売済更新 がつながる
- 推奨コミット:
...
- 未実行
Notes:
- `/qr` に販売済更新確認ダイアログ、任意の購入者選択、販売済更新成功メッセージを追加した。
- 既存の lookup 導線は維持し、`canSell=true` の場合のみ更新できるようにした。
- `todo.md` の次候補は `CUSTOMER-02A` に更新する。
Next:
- CUSTOMER-02A
```
...
- 作業:
  - [x] `PRODUCT_UPDATED` を記録する
  - [x] `PRODUCT_DELETED` を記録する
  - [x] `CUSTOMER_UPDATED` を記録する
  - [x] `CUSTOMER_ARCHIVED` を記録する
  - [x] `QR_SOLD` を記録する
- Done:
  - [x] 主要業務操作ログが Firestore に残る
- 推奨コミット:
  - `feat(api): add operation logs for core mutations`
- 現状メモ: 商品更新、商品論理削除、QR販売済更新で `operationLogs` を記録するようにし、`PRODUCT_UPDATED` / `PRODUCT_DELETED` / `QR_SOLD` を追加した。
...
- 作業:
  - [x] `ERROR` ログを書き込む
  - [x] 原因調査に必要な最小 detail を残す
- Done:
  - [x] 主要エラー発生時に `ERROR` が記録される
- 推奨コミット:
  - `feat(api): add error operation log`
- 現状メモ: 500 系の `AppError` と予期しない例外で `ERROR` を記録し、`requestPath` / `method` / `errorCode` / `statusCode` を残すようにした。
...
- 依存: `WEB-BASE-02`
- 作業:
  - [x] `error-messages.md` に沿ったコード → 文言変換を実装する
  - [x] 項目エラー表示を実装する
  - [x] 画面上部アラートを実装する
  - [x] 画面内エラーを実装する
  - [x] トースト表示を実装する
...
- 現状メモ: 商品、カテゴリ、タグ、顧客、タスクの入力エラーと API エラーを `UiApiError` 経由で統一文言へ変換し、項目エラー・上部通知・画面内エラーへ反映した。
...
- 作業:
  ...
  - [x] Customers API
  - [x] 顧客別購入履歴 API
  - [x] Product Images API
  - [x] Tasks API
  - [x] Categories / Tags API
  - [x] Dashboard API
  - [x] QR API
  - [x] `soldAt` 制御テスト
  - [x] 商品更新 / QR販売済更新時の顧客紐付けテスト
  - [x] `completedAt` 制御テスト
  - [x] `LOGIN` 記録APIを含む認証エラー系
  - [x] `LOGIN` / `PRODUCT_UPDATED` / `PRODUCT_DELETED` / `QR_SOLD` / `ERROR` の主要 operationLogs 記録確認
- Done:
  - [x] 主要エンドポイントの正常 / 異常系が自動化されている
  - [x] 日時制御と認証系の主要業務ルールが API テストで担保されている
  - [x] 主要 operationLogs の記録整合が API テストで担保されている
- 推奨コミット:
  - `test(api): add integration tests for core endpoints and domain rules`
- 現状メモ: `app.test.ts`、`routes/*.test.ts`、各ドメイン単体テストで Health / Products / Customers / Purchases / Images / Tasks / Categories / Tags / Dashboard / QR / 認証 / `soldAt` / `completedAt` / operationLogs の主要観点を自動化済み。
...
- 作業:
  ...
  - [x] QR 画面
- Done:
  - [x] 主要画面のレンダリングと主要導線が自動化されている
- 現状メモ: `apps/web/src/auth/login-page.test.tsx` を追加し、ログイン画面を含む主要画面のレンダリングと主要導線を自動化した。
- 推奨コミット:
...
- 依存: `BOOT-03`
- 作業:
  - [x] `firebase/firestore.rules` を反映する
  - [x] `firebase/storage.rules` を反映する
  - [x] `firebase/firestore.indexes.json` を反映する
  - [x] Firebase Emulator を起動し、Auth / Firestore / Storage / Hosting の基本疎通を確認する
- Done:
  - [x] rules / indexes の反映手順が確認できている
  - [x] Firebase Emulator が設計どおりのポートで起動する
  - [x] Firebase 反映対象ファイルのパスが TODO 上で明確になっている
- 現状メモ: `firebase/firestore.rules`、`firebase/storage.rules`、`firebase/firestore.indexes.json` は `firebase.json` で参照済みで、エミュレータは Auth `9099` / Firestore `8081` / Storage `9199` / Hosting `5000` で起動確認した。
- 推奨コミット:
...
- 作業:
  - [x] `test_design.md` の観点消化を確認する
  - [x] `test_cases.md` の主要ケース消化を確認する
  - [x] README を実装内容に合わせて更新する
  - [x] 未実装項目を次期課題へ分離する
  - [x] リリース判定エビデンスを保存する
- Done:
  - [x] MVP リリース判断に必要な記録がそろう
- 推奨コミット:
...
- git diff --check
Notes:
- `docs/test_design.md` と `docs/test_cases.md` の主要観点を再確認した。
- `README.md` に次期課題を追記した。
- `docs/release-evidence.md` にリリース判定の証跡を保存した。
Next:
- なし
```
...
現時点のリポジトリでは、**初期リリースに必要なタスク群はすべて完了** である。
...
`TASK-01D` の完了により、`DELETE /api/tasks/:taskId` でタスクを物理削除できるようになった。
`TASK-01E` の完了により、`PATCH /api/tasks/:taskId/completion` で完了状態と `completedAt` を更新できるようになった。
`TASK-02A` の完了により、`/products/:productId/tasks` で対象商品の未完了タスク一覧を参照できるようになった。
`TASK-02B` の完了により、`/products/:productId/tasks` でタスクの追加 / 編集 / 削除ができるようになった。
`TASK-02C` の完了により、`/products/:productId/tasks` で完了済み表示切替と完了 / 未完了切替ができるようになった。
`IMAGE-01` の完了により、画像バッファから表示用・サムネイル用 WebP を生成し、MIME / 10MB / 10枚上限判定を共通利用できるようになった。
`IMAGE-02B` の完了により、`PUT /api/products/:productId/images/:imageId` で画像差し替えができるようになった。
`IMAGE-02C` の完了により、`DELETE /api/products/:productId/images/:imageId` で画像削除と並び補正ができるようになった。
`QR-02B` の完了により、`/qr` でカメラ読み取りと lookup 結果表示ができるようになった。
`QR-02C` の完了により、`/qr` で販売済更新確認ダイアログ、任意購入者選択、販売済更新完了メッセージを扱えるようになった。
`LOG-01C` の完了により、商品更新、商品論理削除、QR販売済更新で `operationLogs` を記録できるようになった。
`LOG-01D` の完了により、500 系の主要エラーも `operationLogs` に記録できるようになった。
`LOG-02` の完了により、フロントの主要画面で統一されたエラーメッセージを表示できるようになった。
`TEST-02` の完了により、主要 API の正常 / 異常系、日時制御、認証、operationLogs の記録を自動化できるようになった。
`TEST-03` の完了により、主要画面のレンダリングと主要導線の自動化が揃った。
`INFRA-01` の完了により、Firebase rules / indexes の配置先と Emulator 起動ポートの整合を確認できた。
`REL-01` の完了により、テスト設計 / テストケース / README / release evidence の確認と次期課題の切り出しが完了した。
現時点では `[~]` の再開対象はなく、**次に着手するタスクはない**。

- 現在の完了: `BOOT-01`, `BOOT-02`, `BOOT-03`, `SHARED-01`, `SHARED-02`, `SHARED-03`, `API-BASE-01`, `API-BASE-02`, `API-BASE-03`, `WEB-BASE-01`, `WEB-BASE-02`, `WEB-BASE-03`, `MASTER-01A`, `MASTER-01B`, `MASTER-01C`, `MASTER-01D`, `MASTER-02A`, `MASTER-02B`, `MASTER-02C`, `MASTER-02D`, `MASTER-03A`, `MASTER-03B`, `PRODUCT-01`, `PRODUCT-02`, `PRODUCT-03`, `PRODUCT-04`, `PRODUCT-05A`, `PRODUCT-05B`, `PRODUCT-05C`, `TASK-01A`, `TASK-01B`, `TASK-01C`, `TASK-01D`, `TASK-01E`, `TASK-02A`, `TASK-02B`, `TASK-02C`, `PRODUCT-06A`, `PRODUCT-06B`, `PRODUCT-06C`, `CUSTOMER-00`, `CUSTOMER-01A`, `CUSTOMER-01B`, `CUSTOMER-01C`, `CUSTOMER-01D`, `CUSTOMER-01E`, `CUSTOMER-01F`, `CUSTOMER-02A`, `CUSTOMER-02B`, `CUSTOMER-02C`, `DASH-01`, `DASH-02`, `QR-01A`, `QR-01B`, `QR-02A`, `QR-02B`, `QR-02C`, `LOG-00`, `LOG-01A`, `LOG-01B`, `LOG-01C`, `LOG-01D`, `LOG-02`, `TEST-01`, `TEST-02`, `TEST-03`, `INFRA-01`, `INFRA-02`, `SEC-01`, `IMAGE-01`, `IMAGE-02A`, `IMAGE-02B`, `IMAGE-02C`, `IMAGE-03A`, `IMAGE-03B`, `REL-01`
- 現在の再開候補: なし
- 現在の最優先: なし
- 現在の保留理由: なし
