...
| 7 | 商品一覧 | リスト | サムネイル、商品名、商品ID、ステータス、特注表示、カテゴリ、更新日時 |
...
| 8 | 特注 | 表示 | 特注フラグの有無 |
| 9 | 販売日時 | 表示 | 販売済時のみ表示 |
| 10 | 購入者情報 | 表示 | 販売済かつ顧客紐付け済みの場合に購入者名を表示 |
| 11 | 購入者詳細導線 | ボタン/リンク | 顧客紐付け済みの場合に顧客詳細画面へ遷移 |
...
| 17 | QR印刷ボタン | ボタン | 表示中のQRコードをA4縦向き、2列×5行の10面で印刷 |
| 18 | 編集ボタン | ボタン | 商品編集へ遷移 |
| 19 | 削除ボタン | ボタン | 商品削除確認を表示 |
| 20 | タスク管理ボタン | ボタン | タスク管理画面へ遷移 |
...
| 11 | 保存ボタン | ボタン | - | 登録/更新実行 |
| 12 | キャンセルボタン | ボタン | - | 前画面へ戻る |
| 13 | 入力エラー表示 | メッセージ | - | 項目ごと/画面上部表示 |
...
| 商品登録 / 編集 / 画像登録 / 代表画像設定 / ステータス変更 / 特注フラグ / 購入者設定 | SCR-05 |
